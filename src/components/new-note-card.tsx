...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    speechRecognition.onresult = (event) => {
      const transcription = Array.from(event.results).reduce((phrase, result) => phrase + result[0].transcript, '')
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
...

              {
                shouldShowOnboarding ? (
                  <p className="text-sm leading-6 text-slate-400">
                    Comece <button type="button" className="font-medium text-lime-400 hover:underline" onClick={handleStartRecording}>gravando uma nota</button> em audio ou se preferir <button type="button" onClick={handleStartEditor} className="font-medium text-lime-400 hover:underline">utlize apenas texto</button>.
                  </p>
                ) : (
                  <textarea
                    value={content}
